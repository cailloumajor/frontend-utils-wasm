...
impl BuildDeno {
    pub(crate) fn run(&self) -> anyhow::Result<()> {
        ...
        let ts_file = ts_path.file_name().expect("path should have a filename");
        ...
        let mut js_file = fs::File::create(&js_path).context("Failed to create JS file")?;
        write!(
            js_file,
            "/* @ts-self-types=\"./{}\" */\n\n",
            ts_file.display()
        )
        .context("Failed to write types reference to the JS file")?;
        js_file
            .write_all(bindgen_out.js().as_bytes())
